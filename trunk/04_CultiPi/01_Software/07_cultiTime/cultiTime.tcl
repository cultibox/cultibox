...
puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] Vérification de l'heure du RPI"

set piHour [clock seconds]
puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure du RPi est [clock format $piHour -format "%b %d %H:%M:%S"]"

# Lecture de l'heure du RTC
set rtcHour [::MCP7940N::readSeconds]
puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure du RTC est [clock format $rtcHour -format "%b %d %H:%M:%S"]"
...
if {$piHour < 1421942284} {
    ...
    puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure du RPi n'est pas bonne"

    # On vérifie maintenant que l'heure du RTC n'est pas déconnante
    if {$rtcHour > 1421942284} {
        puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] Mise à jour de l'heure du RPi"
        
        #date MMDDhhmmYY.ss
        exec date [clock format $rtcHour -format "%m%d%H%M%y.%S"]
        
        puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure est mise à jour"

    } else {
        puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure du RTC n'est pas bonne"
    }
...
proc updateRTCTime {} {
    ...
    if {$piHour > 1421942284} {
        ...
        puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] Sauvegarde de l'heure dans le RTC"
        ...
        puts  "[clock format [clock seconds] -format "%b %d %H:%M:%S"] L'heure du RPi est incorrecte. On attend une minute avant de sauvegarder l'heure"
