...
if [ -f $HOME/.cultibox/backup_cultibox.bak ]; then
    ...
    if [ $? -eq 0 ]; then
        sudo -u /Applications/cultibox/xamppfiles/bin/mysql --defaults-extra-file=/Applications/cultibox/xamppfiles/etc/my-extra.cnf -h 127.0.0.1 --port=3891 -e "DROP DATABASE cultibox;"
...
if [ -f $HOME/.cultibox/backup_joomla.bak ]; then
    ...
    if [ $? -eq 0 ]; then
        sudo -u /Applications/cultibox/xamppfiles/bin/mysql --defaults-extra-file=/Applications/cultibox/xamppfiles/etc/my-extra.cnf -h 127.0.0.1 --port=3891 -e "DROP DATABASE cultibox_joomla;"
