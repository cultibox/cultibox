...
If exist C:\cultibox\run\get_version.bat (
    for /f "delims=" %%i in ('C:\cultibox\run\get_version.bat') do (
		...
		If not "!version:~0,6!" == "" (
			...
			set softv=!ver_tmp:.=!
			If !softv! gtr 0 (
				for %%A in (C:\cultibox\xampp\sql_install\update_joomla-*) do ( 
					set Name=%%~nxA
					set file_name=!Name:~14!
					set file_name=!file_name:~0,6!
					set filev=!file_name:.=!
					If !filev! geq !softv! (
						echo   * Updating Joomla database with file:  %%A
						C:\cultibox\xampp\mysql\bin\mysql.exe --defaults-extra-file="C:\cultibox\xampp\mysql\bin\my-extra.cnf" -h 127.0.0.1 --port=3891 < %%A
					)
				)
	...
	If exist C:\cultibox\xampp\sql_install\update_joomla.sql (
		echo   * Updating Joomla database with file: C:\cultibox\xampp\sql_install\update_joomla.sql
		C:\cultibox\xampp\mysql\bin\mysql.exe --defaults-extra-file="C:\cultibox\xampp\mysql\bin\my-extra.cnf" -h 127.0.0.1 --port=3891 < C:\cultibox\xampp\sql_install\update_joomla.sql
	)
) else (
